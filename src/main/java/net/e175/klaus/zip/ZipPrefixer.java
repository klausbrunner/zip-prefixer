  ...
  */
  static final PatternSpec LFH =
      new PatternSpec(
          ...
          FieldSpec.of(2, "fileNameLength"),
          FieldSpec.of(2, "extraFieldLength"));
  static final PatternSpec ZIP64_EOCDL =
      new PatternSpec(
          ByteOrder.LITTLE_ENDIAN,
  ...
  static final PatternSpec ZIP64_EOCDR =
      new PatternSpec(
          ByteOrder.LITTLE_ENDIAN,
   ...
   * @param prefixes Binary prefixes that will be sequentially written before the original ZIP
   *     file's contents.
   ...
   * @param targetPath Target ZIP file. Must be a writeable ZIP format file, in a writeable
   *     directory with enough space to hold a temporary copy.
   * @param prefixFiles Prefix files that will be sequentially written before the original file's
   *     contents.
   * @return Total number of bytes written as prefixes.
   * @throws IOException on errors related to I/O and ZIP integrity
   * @throws ZipOverflowException If the current ZIP format cannot accommodate the new offsets.
  ...
  /**
   * Prepend prefix(es) to an existing file. This method does not care about file types or contents,
   * it just mechanically glues the bytes together.
   *
   * @param targetPath Target file. Must be writeable, in a writeable directory with enough space to
   *     hold a temporary copy.
   * @param prefixes Binary prefixes that will be sequentially written before the original file's
   *     contents.
   * @return Total number of bytes written as prefixes.
  ...
  /**
   * Prepend prefix files to an existing file. This method does not care about file types or
   * contents, it just mechanically glues the bytes together.
   *
   * @param targetPath Target file. Must be writeable, in a writeable directory with enough space to
   *     hold a temporary copy.
   * @param prefixFiles Prefix files that will be sequentially written before the original file's
  ...
  /**
   * Adjust offsets in ZIP file (with validation). If adjustment is 0, don't write anything, just
   * validate.
   *
   * @param targetPath ZIP file to process.
  ...
  private static Queue<Write> analyseOffsets(
      boolean mustAdjust, long adjustment, SeekableByteChannel channel) throws IOException {
    ...
    if (cdOffset != UINT_MAX_VALUE) {
      if (mustAdjust) {
        cdOffset += adjustment;
        writeQueue.add(eocdr.writeInt("offsetOfStartOfCD", uintBoundsChecked(cdOffset)));
      }
    } else {
      requiresZip64 = true;
    }
  ...
  public static void main(String... args) throws IOException {
    ...
    List<Path> paths =
        Arrays.stream(args, 1, args.length).map(Paths::get).collect(Collectors.toList());
