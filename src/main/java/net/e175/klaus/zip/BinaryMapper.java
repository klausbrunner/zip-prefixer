...
/**
 * A small and very limited toolkit to deal with reading/writing binary files based
 * on (fixed-size) frames, particularly ZIPs. The idea is to create "patterns" (like C structs)
 * and read these as needed. Writes are collected first to be executed later as a batch.
 * <p>
 * There isn't a lot of hand-holding; bounds checking is only performed where it's cheap to do and often with
...
final class BinaryMapper {
    ...
    /**
     * Read bytes at given location, assuming that the given PatternSpec is found there. If this has any magic defined,
    ...
    /**
     * Read bytes at given location, assuming that the given PatternSpec is found there. If this has any magic defined,
     * it's used to validate. This allocates a new ByteBuffer.
     */
